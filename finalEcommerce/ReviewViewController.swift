...
class ReviewViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    */
                            ...
                            if let commentList = json["comment"] as? NSArray {
                                for  i in 0..<commentList.count  {
                                    if let commentObj = commentList[i] as? NSDictionary {
                                        ...
                                        let authorEmail = userDetail?["username"] as? String
                                        self.emailList.append(authorEmail!)
                                        let authorAva = userDetail?["_id"] as? String
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let url = URL(string: "\(connection.link)public/uploads/\(avaList[indexPath.row]).jpg")!
        print(url)
